"""
Command Line Interface (CLI) entry point for the solver library.

Supports all 7 solver commands:
...
from solver.deducer import analyze_dependencies, compute_equivalence_classes
from solver.explorer import FormulaExplorer, calculate_diversity_scores, composite_interestingness
from solver.exporters import LeanExporter, GraphExporter
from solver.kb import get_all_axioms, get_combined_signature
from solver.prover import TheoremProver
from solver.utils.doc_generator import build_markdown_docs
...
def build_parser() -> argparse.ArgumentParser:
    ...
    analyze_parser.add_argument("--db", type=str, default="solver_data.db", help="Path to SQLite database")
...
def cmd_analyze(args: argparse.Namespace, config: SolverConfig) -> int:
    ...
    db_path = args.db_path or (args.db if args.db != "solver_data.db" or Path("solver_data.db").exists() else config.db_path)
...
def cmd_docs(args: argparse.Namespace, config: SolverConfig) -> int:
    ...
    docs_created = build_markdown_docs(source_dir="solver", output_docs_dir=output_dir)
